...
=======
title: "Applied_Health_Practicals3"
>>>>>>> ffdce05e27cdec45aa069f55b77b5c4609b13675
...
Pneumonia is an infection that inflames the air sacs in one or both lungs. Pneumonia can be caused by a variety of organisms, including bacteria, viruses, and fungi. An x-ray exam will allow your doctor to see your lungs, heart and blood vessels to help determine if you have pneumonia. When interpreting the x-ray, the radiologist will look for white spots in the lungs (called infiltrates) that identify an infection.Automatic identification of pnuemonia leads to early detection and treatment of pneumonia which are essential for avoiding complications and enhancing clinical results. Also,it allows for faster diagnosis and increased accuracy since the model has been trained and evaluated. It also gives room for scalability since it can be used to screen a large population.
...
The file name tells me the type of pneumonia for a particular child. It is a good one since Pnuemonia is of different types. However, these different variations may impact on predictions since there presentations would be differernt. Hence, it is important that the dataset is carefully distinguished for these variations so the model can be trusted to deliver accurately in terms of predictions for the different types of pnuemonia.
...
for (i in 1:4) {
  # Read the image
  image <- readJPEG(shuffled_train_dataset[i])
  
  # Convert the image to a raster object
  raster <- rasterGrob(image, interpolate = TRUE)
  # Create a ggplot object for the image with the corresponding label
  plot <- ggplot() +
    theme_void() +
    annotation_custom(
...
Yes. Normalization is a data preprocessing step which entails the process of adjusting the numeric values of features to a common scale, without distorting differences in the ranges of values. After examining the three images, I noticed variations in their sizes/brightness levels. Image below to the left appears bigger/darker compared to others, and this might affect the model's ability to generalize well across all images.Also, we could resize across all images. There is need to normalize the brightness/intensities/sizes across all the images to a consistent range.
...
Data Collection: Gather more data for the minority class if possible to balance the dataset naturally.
...
# Generate dummy data
set.seed(123)
train_X <- array(runif(100 * 1 * 224 * 224), dim = c(100, 1, 224, 224))
train_y <- sample(0:9, 100, replace = TRUE)
test_X <- array(runif(20 * 1 * 224 * 224), dim = c(20, 1, 224, 224))
test_y <- sample(0:9, 20, replace = TRUE)
val_X <- array(runif(20 * 1 * 224 * 224), dim = c(20, 1, 224, 224))
val_y <- sample(0:9, 20, replace = TRUE)
...
print(acc_plot)
```
Not working well. To be skipped; 4, 5 and 10
...
Data augmentation involves modifying existing data or creating new data from existing samples. This process expands the dataset, which can help improve how well a model generalizes to new situations it might face in real-world use.It results in an increase of the actual amount of data available. Augumenting images would result in a bigger dataset that will generalize in a better manner to situations that the model could encounter in production.Drawbacks include; 
1.Augmented datasets will carry the biases of the existing datasets. It is important to develop strategies to avoid bias. 
2.Systems need to be put in place to assess and evaluate the quality of augmented datasets. 
# Mobile net
...
ImageNet, officially known as the "ImageNet Large Scale Visual Recognition Challenge 2012," is a widely recognized dataset and competition in the field of computer vision. It was introduced to stimulate research in image classification and object detection by providing a large-scale dataset for training and evaluating machine learning models.

ImageNet-1K contains 1,281,167 training images, 50,000 validation images and 100,000 test images. The full original dataset is referred to as ImageNet-21K. ImageNet-21k contains 14,197,122 images divided into 21,841 classes.  mageNet's vast collection of images and categories provides a diverse and comprehensive training ground for machine learning models. 

Training on such a large-scale dataset would help our models learn general features and patterns that are applicable across various real-world scenarios.Also, by training models on ImageNet, we can create robust systems capable of understanding and interpreting visual data in across diverse applications like medical imaging to facial recognition and natural language processing.
...
MobileNet is one such efficient, light-weighted model in the field of computer which play a significant role in object detection and recognition.
...
MobileNet models pre-trained on large-scale datasets like ImageNet have already learned general features and patterns from a diverse range of images. Therefore, fine-tuning a pre-trained MobileNet on a specific dataset for a practical assignment allows the model to quickly adapt and learn task-specific features, thereby improving results more efficiently than training from scratch. It has powerful object detection and image classification capabilities that would enhance the efficiency and speed of results generation for this practical.

**9_** How many parameters does this network have? How does this compare to better performing networks available in torch? Counting depthwise and pointwise convolutions as separate layers, a MobileNet has 28 layers. A standard MobileNet has 4.2 million parameters which can be further reduced by tuning the width multiplier hyperparameter appropriately. The size of the input image is 224 × 224 × 3. MobileNet's small parameter count and efficient design allow for deployment on mobile and edge devices without compromising too much on accuracy. 
ResNet, VGG, Inception, DenseNet which are the networks available in torch have very high parametes which makes them slower in speed for implemention. MobileNet has a good balance of attaining accuracy and size of parameters which informs its efficiency.
